...
def get_train_test_labels(a):
    ...
    for i in range(8):
        if i not in test:
            train.append(i)
    return train, test
train_large_label, test_large_label = get_train_test_labels(sys.argv[2])
train_label_num = len(train_large_label)
test_label_num = len(test_large_label)
...
if train:
    ...
    for i in range(100):
        ...
        if should_record(i):
            tl.files.save_npz(network.all_params, get_filename(), sess)
            #tl.files.save_npz(network.all_params, params_file_name + 'cos057' + str(lam), sess)
else:
    params = tl.files.load_npz(name=get_filename() + '.npz')
